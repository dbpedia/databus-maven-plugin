...
trait Properties extends Locations with Parameters with Mojo {
   ...
   */
  @Parameter(property = "databus.license", required = true)
  val license: String = null

  /**
   * URL to the main issue tracker for reporting data errors
   */
  @Parameter(property = "databus.issueTracker", required = false)
  val issueTracker: URL = null

  /**
   * URL to the main feedback channel for questions and comments (e.g. "+1" )
   */
  @Parameter(property = "databus.feedbackChannel", required = false)
  val feedbackChannel: URL = null
   ...
   */
  @Parameter(property = "databus.codeReference", required = false)
  val codeReference: URL = null
   ...
   */
  @Parameter(property = "databus.gitCommitLink", required = false)
  val gitCommitLink: URL = null
   ...
   */
  @Parameter(property = "databus.documentationLocation", required = false)
  val documentationLocation: URL = null

  /**
   * default today
   */
  @Parameter(property = "databus.issuedDate")
  val issuedDate: String = null

  @Parameter(property = "databus.tryVersionAsIssuedDate", defaultValue = "false")
  val tryVersionAsIssuedDate: Boolean = false

  /**
   * default from file, else today
   */
  @Parameter(property = "databus.modifiedDate")
...
object Properties {
  ...
  val logo =
    s"""|
        ...
        |######  #    #   #   #    # #####   ####   ####
        |
        |# Plugin version $DatabusPluginVersion - https://github.com/dbpedia/databus-maven-plugin
        |
        |""".stripMargin

  def printLogoOnce(mavenlog: Log) = synchronized {
    if (!logoPrinted) {
      mavenlog.info(logo)
    }
    logoPrinted = true
